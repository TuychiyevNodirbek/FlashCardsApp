...
class DeckRepository(private val deckDao: DeckDao) {
    ...
    suspend fun updateDeck(deck: Deck) =
        deckDao.updateDeck(deck.toEntity().copy(updatedAt = Clock.System.now().toEpochMilliseconds()))

    suspend fun deleteDeck(deck: Deck) = deckDao.deleteDeck(deck.toEntity())

    suspend fun softDeleteDeck(id: String) = deckDao.softDeleteDeck(id, Clock.System.now().toEpochMilliseconds())
